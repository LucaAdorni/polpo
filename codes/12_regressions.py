...
# Drop NAs
def do_logit(df, col, treat = 0, constant = 1):
    """
    ...
    if treat == 1:
        ...
        # # If treat == 1, we will also use dummies for our treated category
        # df_reg = pd.concat([df[col], df.scree_name, pd.get_dummies(df.dist), df.treat, pd.get_dummies(df.dist_treat)], axis = 1)
        # # Set to zero values not belonging to most_region
        # for coltreat in df.dist_treat.unique():
        #     df_reg.loc[df_reg.treat == 0, coltreat] = 0
        # # Remove the first week of COVID-19
        # df_reg.drop(columns = ['0 days', '0 days_treat'], inplace = True)
